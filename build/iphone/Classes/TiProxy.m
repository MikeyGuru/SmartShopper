...
//Rare exceptions to indicate a bug in the _smartshopper code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_smartshopper = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_smartshopper]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.smartshopper.module"
